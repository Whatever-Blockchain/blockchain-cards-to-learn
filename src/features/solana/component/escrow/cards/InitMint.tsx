...
function InitMint({escrowTool}: EscrowToolBox) {
    ...
    async function createMints() {
        ...
        const lamportsForMint = await token.getMinimumBalanceForRentExemptMint(provider.connection);
        ...
        transactionMintA.recentBlockhash = await (await connection.getLatestBlockhash()).blockhash;
        transactionMintA.sign(mintAKeypair); // mintA의 keypair도 필요하다

        const signatureMintA = await provider.wallet.signTransaction(transactionMintA);
        ...
        transactionMintB.recentBlockhash = await (await connection.getLatestBlockhash()).blockhash;
        transactionMintB.sign(mintBKeypair); // mintA의 keypair도 필요하다

        const signatureMintB = await provider.wallet.signTransaction(transactionMintB);
    ...
    return (
        <div className="featuredItem">
            <span className="featuredTitle">[ A. Init Program State ] 2. Mint A and B </span>
            <Button style={{float: 'right', marginRight: '0px'}} variant="contained" onClick={createMints}>
                button
            </Button>
